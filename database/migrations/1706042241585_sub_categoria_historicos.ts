...
/**
 * Classe de migração para criar a tabela 'sub_categoria_historico'.
 *
 * Esta migração verifica se a tabela 'sub_categoria_historico' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'sub_categoria_historico'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'sub_categoria_historico' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('categoria_historico_id').notNullable().unsigned().references('id').inTable('public.categoria_historico').onDelete('NO ACTION').onUpdate('NO ACTION')
   ...
   * Exclui a tabela 'sub_categoria_historico' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
