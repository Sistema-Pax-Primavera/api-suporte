...
/**
 * Classe de migração para criar a tabela 'item'.
 *
 * Esta migração verifica se a tabela 'item' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'item'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'item' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('categoria_item_id').notNullable().unsigned().references('id').inTable('public.categoria_item').onDelete('NO ACTION').onUpdate('NO ACTION')
   ...
   * Exclui a tabela 'item' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
