...
/**
 * Classe de migração para criar a tabela 'raca'.
 *
 * Esta migração verifica se a tabela 'raca' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'raca'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'raca' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('especie_id').notNullable().unsigned().references('id').inTable('public.especie').onDelete('NO ACTION').onUpdate('NO ACTION')
   ...
   * Exclui a tabela 'raca' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
