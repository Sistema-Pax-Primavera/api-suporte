...
/**
 * Classe de migração para criar a tabela 'bairro'.
 *
 * Esta migração verifica se a tabela 'bairro' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'bairro'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'bairro' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('regiao_bairro_id').nullable().unsigned().references('id').inTable('cobranca.regiao_bairro').onDelete('SET NULL').onUpdate('SET NULL')
   ...
   * Exclui a tabela 'bairro' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
