...
/**
 * Classe de migração para criar a tabela 'historico_solicitacao'.
 *
 * Esta migração verifica se a tabela 'historico_solicitacao' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'historico_solicitacao'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'historico_solicitacao' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('solicitacao_id').notNullable().unsigned().references('id').inTable('public.solicitacao').onDelete('NO ACTION').onUpdate('NO ACTION')
   ...
   * Exclui a tabela 'historico_solicitacao' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
