...
/**
 * Classe de migração para criar a tabela 'contato'.
 *
 * Esta migração verifica se a tabela 'contato' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'contato'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'contato' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('tipo').notNullable().defaultTo(5).comment('1-Telefone 2-Celular 3-Email 4-WhatsApp 5-Outros')
   ...
   * Exclui a tabela 'contato' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
