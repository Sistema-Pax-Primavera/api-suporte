...
/**
 * Classe de migração para criar a tabela 'categoria_suporte'.
 *
 * Esta migração verifica se a tabela 'categoria_suporte' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'categoria_suporte'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'categoria_suporte' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.specificType('setor','integer[]').nullable()
   ...
   * Exclui a tabela 'categoria_suporte' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
