    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          table.integer('desconto_id').notNullable().unsigned().references('id').inTable('cobranca.desconto_regra').onDelete('CASCADE').onUpdate('CASCADE')
          ...
          table.primary(['desconto_id', 'unidade_id'])
        })
    }
  }
