    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('tipo').notNullable().comment('Indica o tipo da negociacao: 1-Cobrador  2-Atendimento')
          table.integer('status').notNullable().defaultTo(0).comment('Status da negociacao: 0-Pendente  1-Finalizado')
