...
/**
 * Classe de migração para criar a tabela 'banco'.
 *
 * Esta migração verifica se a tabela 'banco' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'banco'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'banco' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.boolean('ativo').notNullable().defaultTo(true).comment('Se valor for TRUE o mesmo não aparece nas listagens, exceto nas bancos de busca geral.')
   ...
   * Exclui a tabela 'banco' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
