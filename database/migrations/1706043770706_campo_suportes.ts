...
/**
 * Classe de migração para criar a tabela 'campo_suporte'.
 *
 * Esta migração verifica se a tabela 'campo_suporte' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'campo_suporte'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'campo_suporte' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('categoria_id').notNullable().unsigned().references('id').inTable('public.campo_suporte').onDelete('NO ACTION').onUpdate('NO ACTION')
          table.string('descricao').notNullable()
          table.string('tipo').notNullable()
          table.specificType('opcoes', 'json[]').nullable()
   ...
   * Exclui a tabela 'campo_suporte' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
