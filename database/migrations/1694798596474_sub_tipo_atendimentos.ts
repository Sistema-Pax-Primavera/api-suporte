...
/**
 * Classe de migração para criar a tabela 'sub_tipo_atendimento'.
 *
 * Esta migração verifica se a tabela 'sub_tipo_atendimento' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'sub_tipo_atendimento'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'sub_tipo_atendimento' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('tipo_atendimento_id').notNullable().unsigned().references('id').inTable('public.tipo_atendimento').onDelete('NO ACTION').onUpdate('NO ACTION')
   ...
   * Exclui a tabela 'sub_tipo_atendimento' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
