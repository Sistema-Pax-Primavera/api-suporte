...
/**
 * Classe de migração para criar a tabela 'regiao'.
 *
 * Esta migração verifica se a tabela 'regiao' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'regiao'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'regiao' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('cobrador_id').notNullable().unsigned().references('id').inTable('cobranca.cobrador').onDelete('NO ACTION').onUpdate('NO ACTION')
   ...
   * Exclui a tabela 'regiao' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
