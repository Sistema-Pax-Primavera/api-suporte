...
/**
 * Classe de migração para criar a tabela 'historico_financeiro'.
 *
 * Esta migração verifica se a tabela 'historico_financeiro' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'historico_financeiro'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'historico_financeiro' se ela não existir.
   *
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('financeiro_id').notNullable().unsigned().references('id').inTable('financeiro.financeiro').onDelete('NO ACTION').onUpdate('NO ACTION')
   ...
   * Exclui a tabela 'historico_financeiro' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
